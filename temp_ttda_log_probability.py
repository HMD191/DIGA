...
def get_prob_truthground(preds: torch.Tensor, target: torch.Tensor, num_classes: int = 19, ignore_index: int = 255):
	"""
	Get predicted probabilities of the correct class, grouped by ground truth class.
	Returns:
			class_probs: list of length C, where each element is a 1D tensor of probabilities for that class.
	"""
	# preds: [B, C, H, W], target: [B, H, W] 
	target_size = target.shape[-2:]
	preds = nn.Upsample(size=target_size, mode='bilinear')(preds)
	B, C, H, W = preds.shape
	
	preds = torch.softmax(preds, dim=1)  # Convert logits to probabilities

	# Flatten tensors
	probs = preds.permute(0, 2, 3, 1).reshape(-1, C)   # shape: [B*H*W, C]
	labels = target.view(-1)                          # shape: [B*H*W]

	class_probs = [[] for _ in range(num_classes)]  # list of lists, one for each class

	for i in range(labels.shape[0]):
		class_idx = int(labels[i].item())
		if class_idx != ignore_index and 0 <= class_idx < num_classes:
			prob = probs[i, class_idx].item()
			class_probs[class_idx].append(prob)

	return class_probs  # list of tensors
...
class DIGA(LightningModule):
	def __init__(
		...
		self.all_probs_per_class = [[] for _ in range(self.hparams.dataset_info["num_classes"])]
	...
	def _write_class_scores_to_csv(self, step, scores, filename="class_scores.csv"):
		file_exists = os.path.isfile(filename)
		with open(filename, "a", newline="") as f:
			writer = csv.writer(f)

			if not file_exists:
				header = ["step"] + [f"class_{i}" for i in range(len(scores))]
	...
	def forward_and_adapt(self, pair):
		...
		# output_probs = torch.softmax(outputs, dim=1)  # (B, C, H, W)

		probability = get_prob_truthground(preds=outputs,
			target=y, 
			num_classes=self.hparams.dataset_info["num_classes"], 
			ignore_index=255
		)
		...
		class_medians = []
		for class_probs in probability:
			if len(class_probs) > 0:
					median = np.median(class_probs)
			else:
					median = float('nan')  # Or 0.0 if you prefer
			class_medians.append(median)
		
		self._write_class_scores_to_csv(self.test_step_global, class_medians)
		...
		for c in range(self.hparams.dataset_info["num_classes"]):
			if len(probability[c]) > 0:
				self.all_probs_per_class[c].extend(probability[c])
			
			if len(self.all_probs_per_class[c]) > MAX_PER_CLASS:    
				self.all_probs_per_class[c] = random.sample(self.all_probs_per_class[c], MAX_PER_CLASS)
	...
	def test_epoch_end(self, outputs: List[Any]):
		global_medians = []
		for class_probs in self.all_probs_per_class:
			if len(class_probs) > 0:
				median = np.median(class_probs)
			else:
				median = float('nan')
			global_medians.append(median)
		self._write_class_scores_to_csv(self.test_step_global + 2, global_medians)
